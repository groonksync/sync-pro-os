...
const ProductCard = ({ p, onEdit, onDelete, onSelect, isDark }) => {
  ...
  return (
    <div onClick={() => onSelect(p)} className={`${isDark ? 'bg-[#121212] border-white/5 hover:border-blue-500/30' : 'bg-white border-neutral-200 hover:border-blue-500/50'} border rounded-[32px] overflow-hidden transition-all flex flex-col group relative shadow-xl h-full cursor-pointer`}>
      <div className={`${isDark ? 'bg-[#080808]' : 'bg-neutral-50'} aspect-square relative overflow-hidden flex items-center justify-center border-b ${isDark ? 'border-white/5' : 'border-neutral-100'} shadow-inner`}>
        {p.imagen ? (
          <img src={p.imagen} className={`w-full h-full object-cover transition-transform duration-1000 group-hover:scale-110 ${isAgotado ? 'grayscale opacity-30 blur-[2px]' : ''}`} alt={p.nombre}/>
        ) : (
        ...
        {isAgotado && (
           <div className="absolute inset-0 z-40 flex items-center justify-center overflow-hidden">
              <div className="bg-black/60 backdrop-blur-md border-y border-white/10 py-2.5 w-[400%] -rotate-[15deg] shadow-2xl flex items-center justify-center">
                 <span className="text-white text-xs font-black tracking-[1em] uppercase opacity-90">AGOTADO</span>
              </div>
           </div>
        )}
        <div className="absolute top-4 left-4 z-10">
          <span className="text-[8px] text-white font-black uppercase tracking-widest bg-blue-600/90 backdrop-blur-md px-3 py-1.5 rounded-lg border border-white/10 shadow-2xl">{p.categoria || 'Standard'}</span>
        </div>
        ...
        <div className="absolute top-4 right-4 z-10">
          <p className="text-[10px] font-mono font-black px-3 py-1.5 rounded-lg backdrop-blur-md border border-white/10 bg-black/40 text-white shadow-2xl">{p.stock_actual} UDS</p>
        </div>
      </div>
      <div className="p-6 flex flex-col gap-4">
        <div className="space-y-0.5">
           ...
           <div className="flex items-center justify-between">
              <p className="text-[9px] font-black text-blue-500 uppercase tracking-[0.3em]">{p.marca || 'Sovereign Core'}</p>
              <div className={`flex items-center gap-1.5 px-2 py-0.5 rounded-md border text-[7px] font-black uppercase tracking-widest ${p.tipo_envio === 'Envío Gratuito' ? 'bg-emerald-500/10 border-emerald-500/20 text-emerald-500' : 'bg-white/5 border-white/10 text-neutral-500'}`}><Truck size={8} />{p.tipo_envio === 'Envío Gratuito' ? 'Gratis' : 'Adic.'}</div>
           </div>
        </div>
        <div className="flex flex-col">
           <p className="text-[8px] text-neutral-500 font-black uppercase tracking-widest mb-0.5">Venta Final</p>
           <p className={`text-4xl font-mono ${isDark ? 'text-white' : 'text-neutral-900'} font-black tracking-tighter leading-none flex items-baseline`}>{parseFloat(p.precio_venta || 0).toLocaleString()}<span className="text-xs opacity-20 ml-1 font-sans font-black">BS.</span></p>
        </div>
      </div>
    </div>
  );
...
const Inventario = ({ settings, isDark }) => {
  ...
  const handleEditProduct = (p) => {
    ...
    setEditingProduct({ ...p, garantia_num: gNum, garantia_unit: gUnit }); setIsModalOpen(true);
  };

  const openNewProduct = () => {
    setEditingProduct({ 
      id: crypto.randomUUID(), nombre: '', categoria: 'Computadoras', marca: '', precio_costo: 0, precio_venta: 0, precio_antes: 0, stock_actual: 0, stock_minimo: 5, stock_vendido: 0, ficha_tecnica: '', estado: 'Stock', imagen: '', codigo: '', ubicacion: '', serial: '', garantia_num: 0, garantia_unit: 'Días', tipo_envio: 'Envío Gratuito', imagenes: []
    });
    setIsModalOpen(true);
  };
    ...
    <div className={`flex flex-col h-full w-full animate-in fade-in duration-500 ${isMobile ? 'pb-20' : ''}`}>
      ...
      <header className={`flex ${isMobile ? 'flex-col gap-6' : 'flex-row justify-between items-center mb-16 gap-8'}`}>
        <div className={isMobile ? 'text-center w-full' : ''}>
          <h2 className={`text-4xl md:text-6xl font-black ${isDark ? 'text-white' : 'text-neutral-900'} uppercase tracking-tighter leading-none`}>Inventario <span className="text-blue-500">Pro</span></h2>
        ...
        <button onClick={openNewProduct} className={`bg-blue-600 text-white rounded-[2rem] font-black text-[11px] uppercase tracking-widest hover:bg-blue-700 transition-all shadow-2xl flex items-center justify-center gap-4 active:scale-95 ${isMobile ? 'w-full py-6' : 'px-14 py-6'}`}><Plus size={20} strokeWidth={3}/> Nuevo Activo Maestro</button>
      </header>
      ...
      {hasMore && ( <div className="flex justify-center py-20"><button onClick={() => setPage(p => p + 1)} className={`px-10 py-4 ${isDark ? 'bg-white/5 border-white/10 text-neutral-500 hover:text-white' : 'bg-neutral-100 border-neutral-200 text-neutral-500 hover:text-neutral-900'} border rounded-full text-[10px] font-black transition-all uppercase tracking-widest`}>Cargar más activos</button></div> )}

      {/* MODALES Y DETALLE (RESTAURADOS) */}
           ...
           <div className={`${isDark ? 'bg-[#0a0a0a] border-white/10' : 'bg-white border-neutral-200'} border w-full max-w-[1000px] rounded-[48px] p-6 md:p-12 shadow-2xl relative my-auto`}>
              ...
              <div className="grid grid-cols-1 md:grid-cols-12 gap-8 md:gap-12">
                 ...
                 <div className="col-span-12 md:col-span-8 space-y-6">
                    <input type="text" value={editingProduct.nombre} onChange={e=>setEditingProduct({...editingProduct, nombre: e.target.value})} className={`w-full bg-transparent text-2xl font-black ${isDark ? 'text-white' : 'text-neutral-900'} border-b border-white/10 outline-none p-2`} placeholder="Nombre del activo..."/>
                    <div className="grid grid-cols-2 gap-4">
                       <div className="bg-white/5 p-4 rounded-2xl border border-white/5"><p className="text-[7px] font-black text-neutral-500 uppercase mb-2">Marca</p><input type="text" value={editingProduct.marca} onChange={e=>setEditingProduct({...editingProduct, marca: e.target.value})} className="w-full bg-transparent text-sm font-black text-blue-500 outline-none" /></div>
                       <div className="bg-white/5 p-4 rounded-2xl border border-white/5"><p className="text-[7px] font-black text-neutral-500 uppercase mb-2">Categoría</p><input type="text" value={editingProduct.categoria} onChange={e=>setEditingProduct({...editingProduct, categoria: e.target.value})} className="w-full bg-transparent text-sm font-black text-blue-500 outline-none" /></div>
      ...
      {selectedProduct && (
         <div className="fixed inset-0 z-[150] bg-black/98 backdrop-blur-2xl flex items-center justify-center p-2 md:p-10 animate-in fade-in duration-500">
            <div className={`${isDark ? 'bg-[#121212] border-white/10' : 'bg-white border-neutral-200'} border w-full max-w-[950px] rounded-[40px] overflow-hidden shadow-2xl relative flex flex-col md:flex-row`}>
               <button onClick={() => setSelectedProduct(null)} className="absolute top-4 right-4 z-[160] w-12 h-12 bg-blue-600 text-white rounded-full flex items-center justify-center shadow-2xl active:scale-90"><X size={20}/></button>
               <div className="w-full md:w-[42%] bg-black/40 p-8 flex items-center justify-center"><img src={allImages[currentImgIndex]} className="max-w-full max-h-[50vh] object-cover rounded-3xl shadow-2xl" /></div>
               <div className="w-full md:w-[58%] p-8 md:p-12 flex flex-col justify-between">
                  <div className="space-y-6">
                     <div><p className="text-[9px] font-black text-blue-500 uppercase tracking-[0.4em] mb-2">{selectedProduct.categoria}</p><h2 className={`text-2xl md:text-4xl font-black ${isDark ? 'text-white' : 'text-neutral-900'} tracking-tighter uppercase leading-tight`}>{selectedProduct.nombre}</h2></div>
                     <div className="bg-white/5 p-8 rounded-[2.5rem] border border-white/5"><p className="text-[8px] font-black text-neutral-500 uppercase mb-2">Venta Final</p><p className={`text-4xl md:text-6xl font-mono ${isDark ? 'text-white' : 'text-neutral-900'} font-black tracking-tighter leading-none`}>{parseFloat(selectedProduct.precio_venta || 0).toLocaleString()}<span className="text-xs opacity-20 ml-2 font-sans">BS.</span></p></div>
                  </div>
                  <div className="mt-8 flex gap-4"><button onClick={() => handleEditProduct(selectedProduct)} className="flex-1 py-5 bg-white/5 border border-white/5 rounded-[2rem] font-black text-[10px] uppercase hover:bg-white hover:text-black transition-all">Editar</button><button onClick={() => window.open(`https://wa.me/${WHATSAPP_NUMBER}?text=Hola! Me interesa: ${selectedProduct.nombre}`, '_blank')} className="flex-[1.5] py-5 bg-blue-600 text-white rounded-[2rem] font-black text-[10px] uppercase shadow-2xl">WhatsApp</button></div>
               </div>
            </div>
         </div>
      )}

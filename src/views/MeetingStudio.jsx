...
import { 
  ...
  Hash, DollarSign, FolderOpen, ExternalLink, Quote, X 
} from 'lucide-react';
...
const MeetingStudio = ({ meetingsList, setMeetingsList }) => {
  ...
  const createMeeting = () => {
    const newMeeting = { id: Date.now(), cliente: newMeetingForm.cliente || 'Cliente', fecha: newMeetingForm.fecha, estado: 'Pendiente', link: newMeetingForm.link, drive: '', presupuesto: newMeetingForm.presupuesto, contenido: '<p><br></p>', insights: [] };
    setMeetingsList([...meetingsList, newMeeting]); setActiveMeeting(newMeeting); setMeetingView('session');
  ...
  const saveMeeting = () => {
    ...
    setMeetingView('list'); setActiveMeeting(null);
  };
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-black/40 border-b border-white/[0.05]">
                  ...
                  <th className="px-4 py-2.5 text-[10px] font-bold text-neutral-500 uppercase tracking-widest">Link Meet</th>
                </tr>
              </thead>
              <tbody>
                {[...meetingsList].sort((a, b) => a.cliente.localeCompare(b.cliente)).map(m => (
                  <tr key={m.id} onClick={() => openMeeting(m)} className="border-b border-white/[0.02] hover:bg-white/[0.02] cursor-pointer transition-colors group">
                    ...
                    <td className="px-4 py-3 text-xs text-neutral-500">
                      {m.link ? <span className="flex items-center gap-1 text-blue-400 hover:underline"><Video size={10}/> Meet</span> : '-'}
                    </td>
                  </tr>
                ))}
              </tbody>
      ...
      {meetingView === 'create' && (
        <div className="animate-in slide-in-from-bottom-4 duration-300 max-w-xl mx-auto w-full mt-6">
          ...
          <div className="bg-[#0a0a0a] border border-white/[0.05] rounded-xl p-8 shadow-2xl">
            ...
            <div className="mt-8">
              <button onClick={createMeeting} className="w-full py-2.5 bg-white text-black text-xs font-bold rounded-md hover:bg-neutral-200 transition-colors">Crear y Abrir Cuaderno</button>
            </div>
          </div>
        </div>
      )}
          ...
          <header className="mb-4 flex flex-col md:flex-row md:items-end justify-between gap-4 pb-3 border-b border-white/[0.05]">
            <div>
              <div className="flex items-center gap-2 mb-1.5">
                <span className="text-[9px] uppercase font-bold tracking-widest text-neutral-400 bg-white/[0.03] border border-white/5 px-1.5 py-0.5 rounded">{activeMeeting.fecha}</span>
                <select 
                  value={activeMeeting.estado} onChange={(e) => setActiveMeeting({...activeMeeting, estado: e.target.value})}
                  className={`text-[9px] uppercase font-bold tracking-widest px-1.5 py-0.5 rounded outline-none cursor-pointer border ${activeMeeting.estado === 'Finalizado' ? 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20' : 'bg-blue-500/10 text-blue-400 border-blue-500/20'}`}
                >
            ...
            <button onClick={saveMeeting} className="px-4 py-2 bg-white text-black text-[11px] font-bold rounded-md hover:bg-neutral-200 transition-colors flex items-center gap-1.5">
            ...
            <div className="col-span-8 flex flex-col bg-[#0a0a0a] border border-white/[0.05] rounded-xl overflow-hidden shadow-sm">
              <div className="px-3 py-2 border-b border-white/5 flex justify-between items-center bg-black/40">
                ...
                <div className="flex items-center gap-1.5 bg-white/[0.02] px-1.5 py-1 rounded border border-white/5">
                  <button onMouseDown={(e) => applyHighlight('rgba(239, 68, 68, 0.3)', e)} className="w-3.5 h-3.5 rounded-sm bg-rose-500 cursor-pointer"></button>
                  <button onMouseDown={(e) => applyHighlight('rgba(59, 130, 246, 0.3)', e)} className="w-3.5 h-3.5 rounded-sm bg-blue-500 cursor-pointer"></button>
                  <button onMouseDown={(e) => applyHighlight('rgba(234, 179, 8, 0.3)', e)} className="w-3.5 h-3.5 rounded-sm bg-yellow-500 cursor-pointer"></button>
                  <div className="w-[1px] h-3 bg-white/10 mx-0.5"></div>
                  <button onMouseDown={clearHighlight} className="text-neutral-500 hover:text-white p-0.5"><Eraser size={12}/></button>
                  <button onMouseDown={initNoteAddition} className="text-amber-500 hover:text-amber-300 p-0.5"><MessageSquare size={12}/></button>
                </div>
              </div>
              <div ref={editorRef} contentEditable="true" suppressContentEditableWarning={true} className="w-full flex-1 p-5 text-neutral-300 font-light text-sm leading-relaxed mac-scrollbar editor-container" dangerouslySetInnerHTML={{ __html: activeMeeting.contenido }} onBlur={() => setActiveMeeting({...activeMeeting, contenido: editorRef.current.innerHTML})}></div>
            </div>
              ...
              <div className="bg-[#0a0a0a] border border-white/[0.05] rounded-xl p-4">
                ...
                <div className="space-y-2.5">
                  <div className="flex items-center bg-black/40 border border-white/5 rounded px-2 py-1.5">
                    ...
                    <input type="number" value={activeMeeting.presupuesto} onChange={e=>setActiveMeeting({...activeMeeting, presupuesto: e.target.value})} className="bg-transparent text-xs text-white outline-none w-full" placeholder="Presupuesto" />
                  </div>
                  <div className="flex items-center bg-black/40 border border-white/5 rounded px-2 py-1.5">
                  ...
                  {activeMeeting.link && (
                     <a href={activeMeeting.link.includes('http') ? activeMeeting.link : `https://${activeMeeting.link}`} target="_blank" rel="noreferrer" className="flex items-center justify-center gap-1 text-[10px] uppercase font-bold bg-white/5 hover:bg-white/10 text-white rounded py-1.5 transition-colors mt-1">Abrir Google Meet <ExternalLink size={10}/></a>
                  )}
                </div>
              </div>
              <div className="bg-gradient-to-b from-amber-500/[0.02] to-transparent border border-amber-500/10 rounded-xl p-4 flex-1 flex flex-col">
                <div className="flex justify-between items-center mb-3">
                  ...
                  <button onClick={addInsight} className="text-neutral-400 hover:text-white p-0.5 bg-white/5 rounded"><Plus size={12}/></button>
                </div>
                <div className="space-y-3 flex-1 overflow-y-auto pr-1 mac-scrollbar">
                  {activeMeeting.insights.map((insight) => (
                    <div key={insight.id} className="p-2.5 bg-black/40 border border-white/5 rounded-lg relative group">
                      <button onClick={() => setActiveMeeting({...activeMeeting, insights: activeMeeting.insights.filter(i=>i.id!==insight.id)})} className="absolute top-1.5 right-1.5 text-neutral-600 hover:text-rose-500 opacity-0 group-hover:opacity-100"><X size={10}/></button>
                      <input type="text" value={insight.text} onChange={(e) => setActiveMeeting({...activeMeeting, insights: activeMeeting.insights.map(i => i.id === insight.id ? {...i, text: e.target.value} : i)})} placeholder="Idea principal..." className="w-full bg-transparent text-[11px] text-white italic font-medium mb-1.5 outline-none pr-3" />
                      <input type="text" value={insight.note} onChange={(e) => setActiveMeeting({...activeMeeting, insights: activeMeeting.insights.map(i => i.id === insight.id ? {...i, note: e.target.value} : i)})} placeholder="Detalle..." className="w-full bg-transparent border-b border-white/10 text-[10px] text-neutral-500 pb-0.5 outline-none focus:border-amber-500/50" />
                    </div>
                  ))}
                </div>
              </div>

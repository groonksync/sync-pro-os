            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                <div className="flex items-center gap-2">
                   ...
                   <select value={activeMeeting.revision_version} onChange={e=>setActiveMeeting({...activeMeeting, revision_version: e.target.value})} className="bg-white/5 border border-white/10 rounded px-2 py-0.5 text-[9px] font-black text-amber-500 uppercase outline-none ml-2">
                     {['V1','V2','V3','FINAL','APROBADO'].map(v => <option key={v} value={v}>{v}</option>)}
                   </select>
                </div>
                <p className="text-[8px] text-neutral-600 font-bold uppercase tracking-widest mt-1">{activeMeeting.fecha} • War Room Suite</p>
              </div>
            </div>
            <div className="flex items-center gap-3">
               <div className="flex items-center gap-3 bg-black border border-white/5 rounded-xl px-4 py-1.5 shadow-2xl">
          ...
          <div className="flex-1 flex p-1.5 gap-1.5 overflow-hidden">
            ...
            <div className="w-[230px] h-full shrink-0 flex flex-col bg-[#050505] border-white/5 overflow-y-auto mac-scrollbar p-1.5 space-y-1.5">
               <div className="bg-[#0a0a0a] border border-white/5 rounded-xl p-2 shadow-xl">
                  ...
                  <select value={activeMeeting.mood || ''} onChange={e=>setActiveMeeting({...activeMeeting, mood: e.target.value})} className="w-full bg-black border border-white/5 rounded-lg p-2.5 text-[11px] text-white font-black uppercase outline-none shadow-inner">
               ...
               <div className="bg-[#0a0a0a] border border-white/5 rounded-xl p-2 shadow-xl">
                  ...
                  <div className="grid grid-cols-4 gap-1">
                    {['7','8','9','/','4','5','6','*','1','2','3','-','0','.','=','+'].map(btn => (
                      <button key={btn} onClick={() => handleCalc(btn)} className="p-1.5 rounded-md text-[11px] font-black bg-white/5 text-neutral-500 hover:text-white transition-all">{btn}</button>
                    ))}
                  </div>
               </div>
               <div className="bg-[#0a0a0a] border border-white/5 rounded-xl p-2 shadow-xl">
                  <p className="text-[10px] text-neutral-700 font-black uppercase mb-1.5 tracking-widest flex items-center gap-2"><Target size={12}/> Objetivos</p>
                  <textarea value={activeMeeting.session_objective || ''} onChange={e=>setActiveMeeting({...activeMeeting, session_objective: e.target.value})} placeholder="Objetivo de la sesión..." className="w-full bg-black border border-white/5 rounded-lg p-2 text-[9px] text-neutral-400 font-bold outline-none h-20 resize-none" />
               </div>
            </div>
            ...
            <div className="flex-1 min-w-0 flex flex-col bg-black border-x border-white/5 p-1.5 space-y-1.5 items-center overflow-hidden">
               <div className="flex w-[95%] max-w-[1100px] bg-[#0a0a0a] border border-white/5 rounded-2xl p-1 shadow-2xl">
               ...
               {sessionTab === 'editor' && (
                 <div className="flex-1 w-[95%] max-w-[1100px] bg-[#0a0a0a] border border-white/5 rounded-[24px] flex flex-col overflow-hidden shadow-2xl relative animate-in zoom-in duration-500">
                      <div className="flex items-center gap-3 px-6 py-4 border-b border-white/5 bg-black/40 shrink-0">
                          ...
                          <div className="flex items-center gap-2.5 px-4 border-r border-white/10 shrink-0">
                             ...
                             <button onMouseDown={(e) => applyHighlight('transparent', e)} className="p-2 text-neutral-700 hover:text-white"><Eraser size={14}/></button>
                          </div>
                          <div className="flex-1 flex items-center gap-2 px-2 overflow-x-auto no-scrollbar">
                             {EDITOR_TAGS.map(tag => (
                               <button key={tag.name} onMouseDown={(e) => insertTag(e, tag.name, tag.bg, tag.text, tag.border)} className="px-3 py-1.5 rounded-lg text-[9px] font-black uppercase tracking-widest shrink-0 transition-all hover:scale-105 active:scale-95 shadow-lg" style={{ backgroundColor: tag.bg, color: tag.text, border: `1px solid ${tag.border}` }}>{tag.name}</button>
                             ))}
                          </div>
                      </div>
            ...
            <div className="w-[260px] border-l shrink-0 bg-black border-white/5 flex flex-col p-1.5 space-y-1.5 overflow-y-auto mac-scrollbar">
               <div className="bg-[#0a0a0a] border border-white/5 rounded-xl p-2 shadow-xl">
                  ...
                  <div className="flex justify-around">
                    <button onClick={()=>togglePlatform('YT')} className={`p-1.5 rounded-lg transition-all ${activeMeeting.platforms?.includes('YT') ? 'text-rose-500 bg-rose-500/10' : 'text-neutral-800'}`}><Youtube size={14}/></button>
                    <button onClick={()=>togglePlatform('IG')} className={`p-1.5 rounded-lg transition-all ${activeMeeting.platforms?.includes('IG') ? 'text-pink-500 bg-pink-500/10' : 'text-neutral-800'}`}><Instagram size={14}/></button>
                    <button onClick={()=>togglePlatform('FB')} className={`p-1.5 rounded-lg transition-all ${activeMeeting.platforms?.includes('FB') ? 'text-blue-500 bg-blue-500/10' : 'text-neutral-800'}`}><Facebook size={14}/></button>
                  </div>
               </div>
               <div className="bg-[#0a0a0a] border border-white/5 rounded-xl p-3 shadow-xl flex flex-col flex-1 min-h-0 overflow-hidden">
                  <div className="flex bg-black rounded-lg p-0.5 mb-2.5 border border-white/5 shrink-0">
                    <button onClick={() => setRightPanelTab('delivery')} className={`flex-1 py-1.5 rounded text-[7px] font-black uppercase transition-all ${rightPanelTab === 'delivery' ? 'bg-white text-black shadow-lg' : 'text-neutral-600'}`}>Delivery</button>
                    <button onClick={() => setRightPanelTab('tasks')} className={`flex-1 py-1.5 rounded text-[7px] font-black uppercase transition-all ${rightPanelTab === 'tasks' ? 'bg-white text-black shadow-lg' : 'text-neutral-600'}`}>Tasks</button>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-1">
                    {(activeMeeting.pipeline || []).map(step => (
                      <button key={step.id} onClick={() => setActiveMeeting({...activeMeeting, pipeline: activeMeeting.pipeline.map(s => s.id === step.id ? {...s, done: !s.done} : s)})} className={`flex items-center gap-1.5 p-1 rounded-lg border transition-all ${step.done ? 'bg-emerald-500/10 border-emerald-500/20 text-emerald-500' : 'bg-white/5 border-white/5 text-neutral-800'}`}>
                        <div className={`w-3 h-3 rounded flex items-center justify-center shrink-0 transition-all ${step.done ? 'bg-emerald-500 text-black' : 'bg-white/5 border border-white/5'}`}>{step.done && <Check size={8} strokeWidth={4}/>}</div>
      ...
      {showAIModal && (
        <div className="fixed inset-0 z-[700] flex items-center justify-center bg-black/90 backdrop-blur-xl p-8">
          <div className="bg-[#0a0a0a] border border-white/10 rounded-[40px] p-16 w-full max-w-2xl shadow-2xl relative overflow-hidden">
            ...
            <textarea autoFocus value={aiPrompt} onChange={e => setAiPrompt(e.target.value)} className="w-full bg-black border border-white/10 rounded-[30px] p-8 text-lg text-white outline-none h-48 resize-none mb-10 placeholder:text-neutral-800" placeholder="Ej: Genera un gancho viral..." />

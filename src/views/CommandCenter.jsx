  ...
  return (
    <div className={`animate-in fade-in duration-500 w-full ${isMobile ? 'space-y-6' : 'space-y-8'}`}>
      ...
      <header className={`flex ${isMobile ? 'flex-col items-center text-center gap-6' : 'justify-between items-end mb-8'}`}>
        ...
        <div className={`flex ${isMobile ? 'flex-col w-full gap-4' : 'gap-8'}`}>
           <div className={`${isMobile ? 'bg-white/5 p-4 rounded-2xl border border-white/5' : 'text-right'}`}>
            <p className={`${isMobile ? 'text-2xl' : 'text-3xl'} font-mono font-black tracking-tighter leading-none`}>
              {totalCapital.toLocaleString()} <span className="text-xs opacity-40 uppercase">Bs.</span>
            </p>
            <p className="text-[9px] text-neutral-500 tracking-widest uppercase mt-2 font-black">Capital en Calle</p>
          </div>
          <div className={`${isMobile ? 'bg-rose-500/5 p-4 rounded-2xl border border-rose-500/10' : 'text-right border-l border-white/10 pl-8'}`}>
            <p className={`${isMobile ? 'text-2xl' : 'text-3xl'} font-mono text-rose-500 font-black tracking-tighter leading-none`}>
              {totalEgresos.toLocaleString()} <span className="text-xs opacity-40 uppercase">Bs.</span>
            </p>
            <p className="text-[9px] text-rose-500/50 tracking-widest uppercase mt-2 font-black">Egresos Mensuales</p>
          </div>
        </div>
      </header>
      ...
      <div className={`grid gap-6 md:gap-8 ${isMobile ? 'grid-cols-1' : 'grid-cols-12'}`}>
        ...
        <div className={`${isMobile ? '' : 'col-span-8'} bg-black/20 border border-white/5 rounded-[2rem] md:rounded-[32px] p-6 md:p-10 shadow-2xl`}>
          <div className="mb-10 flex flex-col md:flex-row md:justify-between md:items-center gap-2">
            <h3 className="text-xl md:text-2xl font-black uppercase tracking-tighter">Cobros de la Semana</h3>
            ...
            {cobrosActivos.length > 0 ? cobrosActivos.map(p => (
              <div key={p.id} className={`flex ${isMobile ? 'flex-col gap-6' : 'items-center justify-between'} p-6 rounded-[2rem] border border-white/5 bg-white/5 hover:bg-white/10 transition-all group relative`}>
                <div className="flex items-center gap-6 flex-1">
                  ...
                  <div>
                    <h4 className="text-base md:text-lg font-black uppercase tracking-tight">{p.nombre}</h4>
                ...
                <div className={`flex items-center ${isMobile ? 'justify-between pt-6 border-t border-white/5' : 'gap-12'}`}>
                  <p className="text-xl md:text-2xl font-mono font-black tracking-tighter">{(parseFloat(p.capital) * (parseFloat(p.interes) / 100)).toFixed(2)} <span className="text-xs opacity-20">BS.</span></p>

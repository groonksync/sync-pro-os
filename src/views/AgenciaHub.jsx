...
const AgenciaHub = ({ meetingsList = [], setMeetingsList, settings = {} }) => {
  ...
  return (
    <div className="flex flex-col h-screen w-full bg-[#0b0c0e] text-white font-sans overflow-hidden">
      {/* NAV MAESTRO */}
      <nav className="h-16 border-b border-white/5 flex items-center justify-between px-8 bg-[#0b0c0e]/80 backdrop-blur-3xl z-[100]">
        ...
        <div className="flex bg-[#121417] p-1 rounded-lg border border-white/5">
      ...
      <div className="flex-1 overflow-hidden relative">
          ...
          <main className="h-full overflow-y-auto p-12 space-y-12 animate-in fade-in duration-500">
            <div className="flex justify-between items-end">
              ...
              <div className="flex items-center gap-4">
                <div className="relative w-80">
                  ...
                  <input type="text" value={clientSearch} onChange={e=>setClientSearch(e.target.value)} placeholder="Filtrar Identidad..." className="w-full bg-[#121417] rounded-2xl py-4 pl-14 pr-6 text-[11px] outline-none border border-white/5 focus:border-[#10b981]/30 transition-all font-bold uppercase tracking-widest" />
                </div>
            ...
            <div className="grid grid-cols-5 gap-4">
              {clients.filter(c => c.nombre.toLowerCase().includes(clientSearch.toLowerCase())).map((client) => (
                <div key={client.id} className="bg-[#121417] rounded-3xl p-6 border border-white/5 hover:border-[#10b981]/40 transition-all group cursor-pointer" onClick={() => { setActiveClient(client); setViewState('session-list'); }}>
                   <div className="flex justify-between items-start mb-6">
                      <div className="w-12 h-12 bg-[#0b0c0e] rounded-2xl border border-white/10 flex items-center justify-center group-hover:scale-110 transition-transform">
                   ...
                   <button className="w-full py-3 bg-[#0b0c0e] rounded-xl text-[8px] font-black uppercase tracking-widest text-neutral-600 group-hover:text-white group-hover:bg-[#10b981] transition-all">Panel Control</button>
                </div>
              ))}
            </div>
          </main>
          ...
          <main className="h-full overflow-y-auto p-12 space-y-12 animate-in slide-in-from-right duration-500">
             ...
             <div className="grid grid-cols-4 gap-6">
                {meetings.filter(m => m.client_id === activeClient.id).map(meeting => (
                  <div key={meeting.id} onClick={() => { setActiveMeeting(meeting); setViewState('session'); }} className="bg-[#121417] p-8 rounded-[2rem] border border-white/5 hover:border-white/20 transition-all cursor-pointer group">
                     <div className="w-12 h-12 bg-[#0b0c0e] rounded-2xl flex items-center justify-center text-neutral-700 mb-6 group-hover:text-[#10b981] transition-colors"><MessageSquare size={24}/></div>
        ...
        {viewState === 'session' && activeMeeting && (
          <div className="h-full flex animate-in zoom-in duration-500 bg-[#0b0c0e]">
            {/* BARRA LATERAL: HERRAMIENTAS */}
            <aside className="w-80 border-r border-white/5 flex flex-col bg-[#121417]/30">
               <div className="p-8 border-b border-white/5">
                  <p className="text-[9px] font-black text-neutral-700 uppercase tracking-widest mb-4">Control de Tiempo</p>
                  <div className="bg-[#0b0c0e] rounded-2xl p-6 border border-white/5 text-center">
               ...
               <div className="p-8 space-y-6 overflow-y-auto mac-scrollbar flex-1">
                  ...
                  <div>
                    ...
                    <div className="bg-[#121417] rounded-2xl p-6 border border-white/5 space-y-4">
                       ...
                       <div className="space-y-2">
                          {['RAW_CONTENT', 'ASSETS', 'FINAL_VIDEOS'].map(f => (
                            <div key={f} className="flex items-center justify-between p-3 bg-[#0b0c0e] rounded-xl group border border-transparent hover:border-[#10b981]/20">
                               <div className="flex items-center gap-3">
            ...
            <main className="flex-1 flex flex-col">
               <header className="h-16 px-8 border-b border-white/5 flex items-center justify-between bg-[#121417]/50 backdrop-blur-3xl">
          ...
          <main className="h-full overflow-y-auto p-12 space-y-12 animate-in fade-in duration-500">
             ...
             <div className="grid grid-cols-4 gap-8">
                {AGENCY_PLANS.map((plan) => (
                  <div key={plan.name} onClick={() => { setActiveAgencyPlan(plan); setViewState('plan-detail'); }} className="bg-[#121417] p-10 rounded-[3rem] border border-white/5 hover:border-[#10b981]/30 transition-all group cursor-pointer relative overflow-hidden h-96 flex flex-col justify-between">
                     <div className="relative z-10">
                        <div className="w-16 h-16 bg-[#0b0c0e] rounded-3xl flex items-center justify-center mb-8 border border-white/10 group-hover:scale-110 transition-transform">
                     ...
                     <button className="w-full py-5 bg-[#0b0c0e] rounded-2xl text-[10px] font-black uppercase tracking-widest text-neutral-600 group-hover:bg-[#10b981] group-hover:text-white transition-all">Gestionar Plan</button>
        ...
        {viewState === 'plan-detail' && activeAgencyPlan && (
           <main className="h-full overflow-y-auto p-12 space-y-12 animate-in slide-in-from-right duration-500">
             ...
             <div className="grid grid-cols-5 gap-4">
                ...
                {[1,2].map(i => (
                  <div key={i} className="bg-[#121417] rounded-3xl p-6 border border-white/5 hover:border-[#10b981]/40 transition-all group cursor-pointer">
                     <div className="flex justify-between items-start mb-6">
                        <div className="w-12 h-12 bg-[#0b0c0e] rounded-2xl border border-white/10 flex items-center justify-center"><UserIcon size={20} className="text-neutral-700"/></div>
                     ...
                     <button className="w-full py-3 bg-[#0b0c0e] rounded-xl text-[8px] font-black uppercase tracking-widest text-neutral-600 group-hover:text-white group-hover:bg-[#10b981] transition-all">Ver Sesiones</button>
                  </div>
                ))}
             </div>
           </main>
      ...
      {showAIModal && (
        <div className="fixed inset-0 z-[1000] flex items-center justify-center bg-black/90 backdrop-blur-2xl p-10 animate-in zoom-in duration-300">
           <div className="bg-[#121417] rounded-[4rem] p-20 w-full max-w-4xl border border-white/5 shadow-2xl">
              ...
              <textarea autoFocus placeholder="Describe la visión para esta producción..." className="w-full bg-[#0b0c0e] rounded-[3rem] p-12 text-xl outline-none h-64 resize-none mb-12 border border-white/5 focus:border-[#10b981]/30 transition-all text-white font-medium" />
      ...
      {isClientModalOpen && (
        <div className="fixed inset-0 z-[900] flex items-center justify-center bg-black/95 backdrop-blur-3xl p-8 animate-in zoom-in duration-300">
          <div className="bg-[#121417] border-t-8 border-[#10b981] rounded-[4rem] w-full max-w-2xl p-16 space-y-12">
            ...
            <div className="space-y-6">
              <input type="text" value={newClient.nombre} onChange={e=>setNewClient({...newClient, nombre: e.target.value})} placeholder="Nombre Completo..." className="w-full bg-[#0b0c0e] rounded-3xl p-8 text-xl outline-none border border-white/5 focus:border-[#10b981]/30 transition-all text-white" />
              <input type="email" value={newClient.email} onChange={e=>setNewClient({...newClient, email: e.target.value})} placeholder="Email Corporativo..." className="w-full bg-[#0b0c0e] rounded-3xl p-8 text-xl outline-none border border-white/5 focus:border-[#10b981]/30 transition-all text-white" />
              <div className="grid grid-cols-2 gap-6">
                <input type="text" value={newClient.pais} onChange={e=>setNewClient({...newClient, pais: e.target.value})} placeholder="País..." className="w-full bg-[#0b0c0e] rounded-3xl p-8 text-xl outline-none border border-white/5 focus:border-[#10b981]/30 transition-all text-white" />
                <input type="text" value={newClient.empresa} onChange={e=>setNewClient({...newClient, empresa: e.target.value})} placeholder="Empresa..." className="w-full bg-[#0b0c0e] rounded-3xl p-8 text-xl outline-none border border-white/5 focus:border-[#10b981]/30 transition-all text-white" />
              </div>
            </div>
            <div className="flex gap-8">

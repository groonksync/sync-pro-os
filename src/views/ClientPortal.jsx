  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-[#141414] flex flex-col items-center justify-center">
        <div className="w-16 h-1 bg-white/10 overflow-hidden rounded-xl mb-4">
      ...
      <header className="fixed top-0 inset-x-0 z-50 bg-[#141414]/60 backdrop-blur-2xl border-b border-white/5 px-10 py-6 flex justify-between items-center">
        ...
        <div className="flex items-center gap-6">
          <div className="hidden md:flex items-center gap-2 px-4 py-2 bg-white/5 rounded-xl border border-white/5">
             <div className="w-2 h-2 rounded-xl bg-emerald-500 animate-pulse"></div>
             <span className="text-[8px] font-black uppercase tracking-widest text-neutral-400">Sistema Online</span>
          </div>
          <div className="w-10 h-10 rounded-xl bg-gradient-to-tr from-amber-500 to-orange-600 p-[1px]">
             <div className="w-full h-full rounded-xl bg-[#141414] flex items-center justify-center overflow-hidden">
            ...
            <div key={session.id} className="bg-[#141414] border border-white/5 rounded-xl overflow-hidden flex flex-col group hover:border-white/20 transition-all duration-500 shadow-2xl">
              ...
              <div className="aspect-video bg-neutral-900 relative overflow-hidden flex items-center justify-center group-hover:bg-[#141414] transition-all">
                ...
                <div className="absolute bottom-8 left-8 right-8 flex justify-between items-end">
                   ...
                   <button className="w-14 h-14 bg-white text-black rounded-xl flex items-center justify-center hover:scale-110 transition-all active:scale-95 shadow-xl">
              ...
              <div className="p-10 space-y-8">
                <div className="flex justify-between items-center">
                   <div className="flex gap-2">
                     {session.platforms?.map(p => (
                       <div key={p} className="p-2 bg-white/5 rounded-xl border border-white/5 text-neutral-600">
                   ...
                   <span className={`px-4 py-1.5 rounded-xl text-[8px] font-black uppercase tracking-[0.2em] ${session.priority === 'ASAP' ? 'bg-rose-500 text-white' : 'bg-white/5 text-neutral-600'}`}>
                      {session.priority}
                   </span>
                </div>
                  ...
                  <div className="grid grid-cols-4 gap-2">
                    {(session.pipeline || []).map((step, sIdx) => (
                      <div key={sIdx} className="space-y-2">
                        <div className={`h-1.5 rounded-xl transition-all duration-1000 ${step.done ? 'bg-emerald-500 shadow-[0_0_10px_rgba(16,185,129,0.5)]' : 'bg-white/5'}`}></div>
                ...
                <div className="pt-6 border-t border-white/5 flex gap-4">
                   <a 
                     ...
                     target="_blank" 
                     rel="noreferrer" 
                     className="flex-1 py-4 bg-white text-black rounded-xl text-[10px] font-black uppercase tracking-widest text-center hover:bg-neutral-200 transition-all flex items-center justify-center gap-2"
                   >
                     <ExternalLink size={14}/> Revisar Versión
                   </a>
                   <button className="px-6 py-4 bg-white/5 border border-white/10 rounded-xl text-white hover:bg-white/10 transition-all">
                     <Download size={16}/>
                   </button>
                </div>
              </div>
